...
@app.post("/api/v1/services/{service_name}/actions", tags=["services"])
def service_action(service_name: str, payload: ServiceAction, request: Request) -> dict[str, str]:
    if service_name not in SUPPORTED_SERVICES:
        raise HTTPException(status_code=400, detail="Unsupported service")
    if service_name == "rabby-host" and payload.action == "stop":
        raise HTTPException(status_code=400, detail="The control panel cannot stop itself through the API")
    try:
